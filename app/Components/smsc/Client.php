...
class Client
{
    ...
    {
        $this->login = env('SMSC_LOGIN');                     //логин клиента
        $this->password = env('SMSC_PASSWORD');               //пароль
        $this->methodPost = env('SMSC_POST');        //использовать метод POST
        $this->useHttps = env('SMSC_HTTPS');               //использовать HTTPS протокол
        $this->charset = env('SMSC_CHARSET');                 //кодировка сообщения: utf-8, koi8-r или windows-1251 (по умолчанию)
        $this->from = env('SMSC_FROM');                //e-mail адрес отправителя
    }
    ...
    public function send_sms($phones, $message, $translit = 0, $time = 0, $id = 0, $format = 0, $sender = false, $query = [], $files = [])
    {
        ...
        $request = $this->request("send", array_merge($query, [
            "cost" => 3,
            "phones" => $phones,
            "mes" => $message,
            "translit" => $translit,
    ...
    {
        $base_url = ($this->useHttps ? "https" : "http")."://smsc.kz/sys/$cmd.php";
